...
module.exports.collapseDb = (loggedUser, serviceName, req) => {
  return new Promise((resolve, reject) => {
    ...
    User.find(query).then(users => {
      if (!users) {
        logger.error(`auth-experimental-collapse-db collapse-db - db error cannot find user`);
      ...
      duplicatedUser = users.filter(dbUser => {
      ...
      saved = savedUser;
      if (!savedUser) {
        logger.error('auth-experimental-collapse-db collapse-db - error while saving collapsed users');
      ...
      return User.findByIdAndRemove(duplicatedUser._id);
    }).then(() => {
      // we have deleted the user
      logger.debug('auth-experimental-collapse-db collapse-db - duplicated User deleted', saved);
      resolve(saved);
    }).catch(err => {
